  ...
  for (int j = 0; j < 3; j++) {
    ...
    for (int l = 0; l < 3; l++) {
      ...
      try {
        ...
        result.push(make_pair(index_of_coincidence(plain_text_candidate),
                              plain_text_candidate));
        cout << j << l << "plain text candidate: " << plain_text_candidate
             << endl;
      }
...
void vigenere_kasiki_attack(string ciphertext) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    for (int j = 0; j < idx_combs.size(); j++) {
      ...
      result.push(make_pair(-abs(index_of_coincidence(plain_text_candidate) -
