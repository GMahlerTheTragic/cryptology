...
int VigenereCipher::get_next_shift(size_t round, string &plain_text,
                                   string &cipher_text) {
    int l = this->keyword.length();
...
int BeaufortCipher::get_next_shift(size_t round, string &plain_text,
                                   string &cipher_text) {
    int l = this->keyword.length();
    return letter_to_pos(this->keyword[mod(round, l)]);
}

int AutoKeyPlainTextCipher::get_next_shift(size_t round, string &plain_text,
                                           string &cipher_text) {
...
int AutoKeyCipherTextCipher::get_next_shift(size_t round, string &plain_text,
                                            string &cipher_text) {
    if (round < this->keyword.size()) {
        return letter_to_pos(this->keyword[round]);
    }
