...
string BeaufortCipher::encrypt(string plain_text) {
    ...
    for (int i = 0; i < plain_text.length(); ++i) {
        uint8_t shift = get_next_shift(i, plain_text, output);
        output[i] = pos_to_letter(Mod(letter_to_pos(shift - output[i]), knumber_of_letters));
    }
    return output;
}

string BeaufortCipher::decrypt(string plain_text) { return this->encrypt(plain_text); }
