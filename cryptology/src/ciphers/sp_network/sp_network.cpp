...
SpNetwork::SpNetwork(const PBox &p_box, const SBox &s_box, size_t block_size, size_t n_rounds,
    ...
    if (block_size % s_box.input_size() != 0) {
        throw std::runtime_error(
            "Block Size has to be a multiple of SBox "
            "size");
    }
    if (s_box.input_size() != s_box.ouput_size()) {
        throw std::runtime_error(
            "Currently SBoxes have to have equal input "
            "and output sizes");
    }
    this->s_box = s_box;
    this->p_box = p_box;
    ...
    this->linear_approximations = this->s_box.compute_linear_approximation_table();
}

size_t SpNetwork::sbox_size() const { return this->s_box.input_size(); }

size_t SpNetwork::block_size() const { return this->block_size_; }

size_t SpNetwork::rounds() const { return this->n_rounds; }
...
DynamicBitset SpNetwork::apply_sbox(const DynamicBitset &input) {
    ...
    for (size_t i = 0; i < this->n_parallel_sboxes(); i++) {
        ...
        DynamicBitset block_result =
            this->s_box.forward(input.slice(offset, offset + this->sbox_size()));
...
DynamicBitset SpNetwork::apply_sbox_inv(const DynamicBitset &input) {
    ...
    for (size_t i = 0; i < this->n_parallel_sboxes(); i++) {
        size_t offset = i * this->sbox_size();
        DynamicBitset block_result =
            this->s_box.backward(input.slice(offset, offset + this->sbox_size()));
        result = result.concat(block_result);
    }
    return result;
}
...
RoundLinearApproximation SpNetwork::compute_best_output_approximation(
    ...
    for (size_t i = 0; i < this->n_parallel_sboxes(); i++) {
        ...
        if (!slice.is_zero()) {
            LinearApproximation best_approx = this->find_best_approximation(slice.to_uint64(), i);
            round_approximations.push_back(best_approx);
            active_outputs =
                active_outputs.concat(DynamicBitset(best_approx.active_outputs, this->sbox_size()));
        } else {
            active_outputs = active_outputs.concat(DynamicBitset(this->sbox_size()));
        }
    }
...
LinearApproximation SpNetwork::find_best_approximation(uint64_t active_i, size_t sbox_pos) const {
    ...
    return {best_bias, best_active_o};
}
